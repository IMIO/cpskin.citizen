...
class CitizenSecurityMigrateView(BrowserView):
    ...
    @property
    def draft_folders(self):
        lrfs = api.content.find(
            context=self.portal,
            portal_type='LRF',
        )
        if not lrfs:
            return [self.portal[utils.DRAFT_FOLDER_ID]]
        return [lrf.getObject()[utils.DRAFT_FOLDER_ID] for lrf in lrfs]

    @property
    def dahsboard_folders(self):
        lrfs = api.content.find(
            context=self.portal,
            portal_type='LRF',
        )
        folders = []
        if not lrfs:
            base_folder = self.portal[utils.DASHBOARD_FOLDER_ID]
    ...
    def _migrate_dashboard_folders(self):
        ...
        for dashboard_folder in self.dahsboard_folders:
